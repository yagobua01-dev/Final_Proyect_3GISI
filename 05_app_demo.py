...
def process_query(text_input, audio_input):
    """
    Handles the user input from Gradio. Transcribes audio if provided,
    otherwise uses the text input. Then runs the A* search pipeline.
    """
    final_query = ""
    status_message = ""
    
    # 1. Handle Speech-to-Text
    if audio_input is not None:
        # Whisper transcribes the audio file generated by the microphone
        result = whisper_model.transcribe(audio_input)
        final_query = result["text"].strip()
        status_message = f"Audio transcribed: '{final_query}'"
    elif text_input:
        final_query = text_input.strip()
        status_message = f"Searching for: '{final_query}'"
    else:
        return [], "Please provide either text or a voice recording."
        
    # 2. Perform Multimodal Search
    query_vector = get_text_embedding(final_query)
    ...
    if best_subset_local_indices:
        for local_idx in best_subset_local_indices:
            ...
            # Gradio gallery expects a list of paths or PIL Images
